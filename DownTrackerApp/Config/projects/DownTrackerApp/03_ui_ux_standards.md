...
- **Sekme ve Rehber Başlık Yapılandırması:**
  - **Ana Ekran Sekmeleri (Vektörel ImageList):** Windows GDI+ / uxtheme boş kutu (□) render hatalarını önlemek amacıyla tüm font tabanlı ikonlar kaldırılmış, yerine runtime GDI+ vektörel çizim motoru (`CreateImageList(16)`) entegre edilmiştir. Sekme metinleri sade ve kurumsal isimlerden oluşur:
    ...
    4. `"Grafana Sorgu Sihirbazı"` (İndex 3: Terminal script prompt `>_`)
    5. `"P360 Import Sihirbazı"` (İndex 4: Sayfa ve aşağı ok)
    6. `"P360 Hatalı Import Sihirbazı"` (İndex 6: Kırmızı X işareti)
    * **İkon Boşluk Standardı:** Sekme butonlarının solundaki ikonların sıkışmasını önlemek için sekme denetleyicisinin iç dolgusu `tabMain.Padding = new Point(18, 6)` olarak yapılandırılmıştır. Bu sayede ikonların solunda asil bir hizalama boşluğu bırakılır. İkon ile metin arasındaki çizim aralığı (`spacing`) ise daha rahat bir okunabilirlik için `8px` olarak kilitlenmiştir.
  - **Rehber Ekranı (GuideForm) ve Sol Kılavuz Menüsü:** 
    * **Dinamik Ekran Adaptasyonu:** Rehber ekranı (`GuideForm`) küçük ekranlarda ve yüksek DPI ölçeklendirmelerinde dikeyde kaybolmasını önlemek için formun başlangıç boyutunu ekranın çalışma alanına göre (`Ekran Yüksekliği - 40px`, min: `800x500`) dinamik olarak uyarlar. Formun kenarlıkları `Sizable` yapılarak kullanıcının serbestçe boyutlandırabilmesi ve ekranı kaplayabilmesi (`MaximizeBox = true`) sağlanmıştır.
    * **Sol Menü Scroll & Dock Standardı:** Sol kılavuz menüsü (`pnlGuideTabs`) `Width = 280` olarak yapılandırılmıştır ve butonlar `Dock = DockStyle.Top` düzenine sahiptir. Bu sayede buton konumları dikeyde otomatik olarak hizalanır ve pencere küçüldüğünde butonlar dikey kaydırma çubuğuyla (`AutoScroll`) üst üste binmeden kusursuz şekilde daralır.
    * **İkon Hizalama ve Padding standardı:** Butonlar 24x24 piksel pürüzsüz vektörel ikonlar içeren `_guideImageList` kullanır. Butonların sol padding değeri `Padding = new Padding(16, 0, 0, 0)` olarak artırılmış ve buton metinlerinin önüne `"  "` çift boşluğu eklenerek ikon ve metin arası görsel denge (`ImageBeforeText` ve `MiddleLeft` hizalaması ile) korunmuştur:
      * `"Nasıl Çalışır / Proses Açıklaması"` (İndex 7: Büyüteç)
      * `"Down Conditions Ekranı"` (İndex 0)
      * `"Çevrim Analizi Ekranı"` (İndex 1)
      * `"Ham Veri Ekranı"` (İndex 2)
      * `"Grafana Sorgu Sihirbazı Ekranı"` (İndex 3)
      * `"P360 Import Sihirbazı Ekranı"` (İndex 4)
      * `"P360 Hatalı Import Sihirbazı"` (İndex 6: X işareti)
