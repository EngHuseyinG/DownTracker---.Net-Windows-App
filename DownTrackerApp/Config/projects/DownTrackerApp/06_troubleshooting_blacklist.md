...
- **Çözüm Algoritması:**
  1. `BindCustomScrollbar` metodunun sarmalayıcı panelin yüksekliğini ezmesi engellenmiş, panelin tasarım anındaki veya oranlanmış asil yüksekliği (`170px`, `135px`, `115px`) korunarak yatay scrollbar panelin en altına mühürlenmiştir (`DockStyle.Bottom`). Bu sayede kaydırma çubuğu grid satırlarının üzerine binmeden en altta render edilir ve veri satırları kırpılmaz.
...
### 7. Teknik Kılavuz (GuideForm) Ekranında Küçük Ekran Kırpılması ve Menü Kaydırma Hatası
- **Sorun:** Küçük ekran çözünürlüklerinde (veya yüksek Windows DPI ölçeklendirmesinde) rehber ekranının (`GuideForm`) alt kısımları ekranın ve taskbar'ın altına taşıyor, form boyutlandırılamıyor (`FixedDialog`) ve sol menü butonları dikeyde taşmasına rağmen kaydırma çubuğu çıkmıyordu.
- **Kök Neden:** 
  1. Formun `FormBorderStyle = FixedDialog` olması ve `MaximizeBox = false` olarak kilitlenmesi nedeniyle kullanıcı pencereyi küçültüp ekran sınırlarına uyduramıyordu.
  2. Form boyutu (`1020x580`) ekranın çalışma alanından büyük kaldığında alt kısımlar ekran dışına taşıyor ancak formun kendisi küçülmediği için `pnlGuideTabs` panelinin yüksekliği daralmıyor ve dolayısıyla dikey kaydırma çubuğu tetiklenmiyordu.
  3. Sol menü butonları manuel mutlak koordinatlarla (`Location = new Point(0, top)`) yerleştirildiği için, dikeyde `AutoScroll` aktif olsa bile butona sabit konum verilmesi nedeniyle kaydırma çubuğu geldiğinde butonların sağ kenarları çubuğun altında kalıp kırpılıyordu.
- **Çözüm Algoritması:**
  1. Formun boyutu yüklenme esnasında ekranın çalışma alanına (`Screen.PrimaryScreen.WorkingArea`) göre dinamik olarak sınırlandırılır (`desiredHeight = Math.Max(500, workingArea.Height - 40)`).
  2. Kenarlık stili `Sizable` yapılır ve ekranı kaplama (`MaximizeBox = true`) özelliği aktif edilir.
  3. Sol menü butonlarının manuel koordinat atamaları tamamen kaldırılıp `Dock = DockStyle.Top` düzenine geçirilir. Bu sayede butonlar dikeyde otomatik olarak sıralanır, dikey kaydırma çubuğu (`AutoScroll`) tetiklendiğinde buton genişlikleri otomatik olarak daralarak kaydırma çubuğunun altında kalıp kırpılmaz.
