...
namespace DownTracker.Views
{
    public class GuideForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            // Adapt to screen size dynamically
            var workingArea = Screen.PrimaryScreen.WorkingArea;
            int desiredWidth = 1020;
            int desiredHeight = 580;
            if (workingArea.Width < desiredWidth)
                desiredWidth = Math.Max(800, workingArea.Width - 40);
            if (workingArea.Height < desiredHeight)
                desiredHeight = Math.Max(500, workingArea.Height - 40);
            ...
            // 1. Nasıl Çalışır
            btnHowItWorks = CreateTabButton("Nasıl Çalışır / Proses Açıklaması", 7);
            ...
            // 2. Down Conditions
            btnDownConditions = CreateTabButton("Down Conditions Ekranı", 0);
            ...
            // 3. Üretim Çevrimleri
            btnProdCycles = CreateTabButton("Çevrim Analizi Ekranı", 1);
            ...
            // 4. Ham Veri
            btnRawData = CreateTabButton("Ham Veri Ekranı", 2);
            ...
            btnGrafanaWizard = CreateTabButton("Grafana Sorgu Sihirbazı Ekranı", 3);
            ...
            // 6. Import Sihirbazı
            btnImportWizard = CreateTabButton("P360 Import Sihirbazı Ekranı", 4);
            ...
            btnFaultyImportGuide = CreateTabButton("P360 Hatalı Import Sihirbazı", 6);
            ...
            btnAboutProduct = CreateTabButton("Uygulama Hakkında / Künye", 5);
        ...
        private Button CreateTabButton(string text, int imageIndex)
        {
            var btn = new Button
            {
                Text = "  " + text,
        ...
        private void ShowCard(string title, string contentText, Color titleColor, int imageIndex = -1)
        {
            ...
            // Scrollable Panel for text content
            ...
            pnlScroll.Controls.Add(lblContent);
            pnlCard.Controls.Add(pnlScroll);
            pnlCard.Controls.Add(lblTitle);

            pnlGuideContent.Controls.Add(pnlCard);

            // Dynamically adjust label height based on text wrapping
            void AdjustLabelHeight()
            {
                int width = pnlScroll.ClientSize.Width - pnlScroll.Padding.Horizontal;
                width -= 22; // Leave some space for the vertical scrollbar
                ...
                lblContent.Height = size.Height + 30; // 30px buffer for vertical padding
            }

            pnlScroll.Resize += (s, e) => AdjustLabelHeight();
            lblContent.Resize += (s, e) => AdjustLabelHeight();
            AdjustLabelHeight();
        }
